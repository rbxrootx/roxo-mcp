//! The process entrypoint, shared by the `roxo` binary and the `rojo` alias
//! that exists so Roxo can drop into a toolchain expecting Rojo.
...
/// Roxo's entrypoint, shared by the `roxo` and `rojo` binaries.
pub fn main() {
    ...
    panic::set_hook(Box::new(|panic_info| {
        ...
        log::error!(
            "Roxo crashed! You are running Roxo {}.",
            env!("CARGO_PKG_VERSION")
        );
        log::error!("This is probably a Roxo bug.");
        log::error!("");
        log::error!(
    ...
    let log_filter = match options.global.verbosity {
        0 => "info",
        1 => "info,libroxo=debug",
        2 => "info,libroxo=trace",
        _ => "trace",
    };
