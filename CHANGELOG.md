...
* **Deleting a directory no longer kills the server.** Removing a directory also removes its children, and each child's event names a parent that is already gone; resolving it panicked, and since the build aborts on panic the whole process died. Carries [rojo#1319] by @michaelmitchell-bit, which fixes it for stale Create, Write and Remove events alike. Related reports: [rojo#1236], [rojo#1206], [rojo#1309].
