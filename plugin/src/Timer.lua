...
function Timer._start(label)
	local start = clock()
	if not label then
		error("[Roxo-Timer] Timer.start: label is required", 2)
		return
	end
...
function Timer._stop()
	...
	if not entry then
		error("[Roxo-Timer] Timer.stop: no label to stop", 2)
		return
	end
	...
	print(string.format("[Roxo-Timer] %s took %.3f ms", label, duration * 1000))
end
